...
export class CampaignsProcessor {
  ...
  async handleSendMessage(job: Job) {
    ...
    try {
      ...
      // Validação CPC do parceiro (Paschoalotto)
      // Buscar contato para pegar o contrato e buscar o nome do segmento
      ...
      if (contactSegment) {
        const segment = await this.prisma.segment.findUnique({
          where: { id: contactSegment },
        });
        segmentName = segment?.name || null;
      }

      // Se tiver contrato e segmento, validar com a API CPC
      if (this.cpcValidationService.isEnabled() && contact?.contract && segmentName) {
        const canContactResult = await this.cpcValidationService.canContact(
          contact.contract,
          contactPhone,
          segmentName,
        );

        if (!canContactResult.allowed) {
          this.logger.warn(
            `CPC bloqueado para ${contactPhone}: ${canContactResult.reason}`,
            'CampaignsProcessor',
            { campaignId, contactPhone, contract: contact.contract, segmentName },
          );
          await this.prisma.campaign.update({
            where: { id: campaignId },
            data: { response: false },
        ...
        try {
          ...
          if (this.cpcValidationService.isEnabled() && contact?.contract && segmentName) {
            try {
              ...
              this.logger.log(
                `CPC registrado para ${contactPhone}`,
                'CampaignsProcessor',
                { campaignId, contactPhone, contract: contact.contract, segmentName },
              );
            } catch (cpcError) {
              // Não bloquear o envio se falhar o registro CPC, apenas logar
              this.logger.warn(
                `Falha ao registrar CPC para ${contactPhone}: ${cpcError.message}`,
                'CampaignsProcessor',
