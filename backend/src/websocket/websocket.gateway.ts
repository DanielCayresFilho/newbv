...
export class WebsocketGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleSendMessage(
    ...
    try {
      ...
      // Validação CPC do parceiro (Paschoalotto) - verifica se cliente pode ser acionado
      if (this.cpcValidationService.isEnabled() && data.isNewConversation) {
        // Buscar contato para pegar contrato
        const contactForCpc = await this.prisma.contact.findFirst({
        ...
        if (contactForCpc?.contract) {
          ...
          if (user.segment) {
            const segmentForCpc = await this.prisma.segment.findUnique({
              where: { id: user.segment },
            });
            segmentName = segmentForCpc?.name || null;
          }
          ...
          if (segmentName) {
            ...
            if (!cpcResult.allowed) {
              ...
              client.emit('message-error', {
                error: cpcResult.reason,
                errorCode: 'CPC_BLOCKED',
      ...
      // Registrar acionamento CPC no parceiro (Paschoalotto)
      // Apenas para 1x1, se habilitado e contato tiver contrato
      if (!isGroup && this.cpcValidationService.isEnabled() && contact?.contract) {
        try {
          // Buscar nome do segmento se necessário
          let segmentName: string | null = null;
          if (user.segment) {
            const segmentInfo = await this.prisma.segment.findUnique({
              where: { id: user.segment },
          ...
          // Apenas logar erro, não interromper fluxo pois mensagem já foi enviada
          console.error(`❌ [WebSocket] Falha ao registrar CPC: ${cpcError.message}`);
        }
      }
